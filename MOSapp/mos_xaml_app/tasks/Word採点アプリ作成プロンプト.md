...
## 概要
現在のMOS_XAML_APP（Excel採点アプリ）と同様の構造で、Word採点アプリを作成してください。UIの配置や機能はExcel版と同じですが、起動するアプリケーションとCheckerがWord用に変更されます。

**重要**: このプロジェクトは別フォルダ、別のCursorワークスペースで作成してください。Excel版のコードをコピーして、以下の変更を適用してください。
...
- **メインコンテンツエリア（下部）**:
  - ナビゲーション: 前へボタン（←）、タスク番号ボタン（実際のWordの問題数に合わせる）、次へボタン（→）
  - 問題文表示エリア: 現在のタスクの問題文を表示（"で囲まれた部分に下線を付けて表示、クリックでクリップボードにコピー）
  - アクションボタン: 解答済みボタン、あとで見直すボタン（フラグ）

**重要**: タスク番号ボタンの数は、Excel版の「1-7」ではなく、**Word版の実際の問題数に合わせてください**。各プロジェクトの問題数は異なる可能性があります。問題数はCSVファイルで管理されます（のちほど作成）。
...
**変更後（Word）**:
```csharp
private void LaunchAndPositionWord(int appBarHeight)
{
    // Word起動処理
    string[] candidates = new[]
    {
...
**変更後（Word）**:
```csharp
string expectedPath = @"C:\MOSTest\Word365\Tab1\project2.docx";
// または適切なWordファイルのパス
```
...
**変更後（Word）**:
- `MOS演習問題文一覧_Word.json`（または適切な命名規則）
- `MOS模擬試験①問題文一覧_Word.json`
- `MOS模擬試験②問題文一覧_Word.json`
...
**変更後（Word）**:
```csharp
// Libraries/Group1/WordChecker1_2.cs など（のちほど作成）
string libraryName = "WordChecker1_2";
string dllPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Libraries", GetGroupFolder(libraryName), $"{libraryName}.dll");
Type checkerType = assembly.GetType(libraryName);
```
...
**変更後（Word）**:
- `.docx`, `.docm` などのWordファイル拡張子

### 9. アプリバー内の問題文表示
- JSONファイルから読み込む問題文はWord用の内容に変更
- 問題文の表示ロジック自体は変更不要（`SetTextWithUnderline`メソッドなど）

### 10. レビュー画面の問題文表示
- 同様にWord用のJSONファイルから読み込む
- 表示ロジックは変更不要

## プロジェクトファイル（.csproj）の変更

### プロジェクト名と名前空間の変更
**変更前（Excel）**:
```xml
<RootNamespace>MOSExcelMogiApp</RootNamespace>
<AssemblyName>MOSExcelMogiApp</AssemblyName>
```

**変更後（Word）**:
```xml
<RootNamespace>MOSWordMogiApp</RootNamespace>
<AssemblyName>MOSWordMogiApp</AssemblyName>
```
...
### ファイルパスの変更（PublishUrlなど）
**変更前（Excel）**:
```xml
<PublishUrl>C:\Users\kouza\Desktop\Excelアプリ\</PublishUrl>
```

**変更後（Word）**:
```xml
<PublishUrl>C:\Users\kouza\Desktop\Wordアプリ\</PublishUrl>
```
...
**重要**: 
- `excelFile` → `wordFile` に変更し、ファイルパスも `.xlsx` → `.docx` に変更
- **`taskCount`の値は、Excel版の値ではなく、Word版の実際の問題数に合わせてください**。各プロジェクトごとに問題数が異なる可能性があります。
...
すべてのC#ファイルで以下の名前空間を変更してください：

**変更前（Excel）**:
- `namespace MOSExcelMogiApp`
- `namespace MOSExcelMogiApp.Views`
- `namespace MOSExcelMogiApp.Ui.ViewModels`
- `namespace MOSExcelMogiApp.Core.Adapters`
- `namespace MOSExcelMogiApp.Infrastructure`
- `namespace Libraries.Group1`（Checkerクラス用）

**変更後（Word）**:
- `namespace MOSWordMogiApp`
- `namespace MOSWordMogiApp.Views`
- `namespace MOSWordMogiApp.Ui.ViewModels`
- `namespace MOSWordMogiApp.Core.Adapters`
- `namespace MOSWordMogiApp.Infrastructure`
- `namespace Libraries.Group1`（Checkerクラス用、名前空間は同じでもOK）
...
├── AppBarWindow.xaml / AppBarWindow.xaml.cs
├── Assets/
│   └── config.json
├── Core/
│   ├── Adapters/
│   │   └── WordCheckerService.cs（ExcelCheckerService.csから変更）
│   ├── Models/
│   │   └── ProjectInfo.cs
│   └── Ports/
│       ├── Primary/
│       │   └── IWordCheckerService.cs（IExcelCheckerService.csから変更）
│       └── Secondary/
│           └── IWordCheckerRepository.cs（IExcelCheckerRepository.csから変更）
├── Infrastructure/
│   └── WordCheckerRepository.cs（ExcelCheckerRepository.csから変更）
├── Libraries/
│   ├── Group1/
│   │   ├── WordChecker1_1.cs（のちほど作成）
│   │   ├── WordChecker1_2.cs（のちほど作成）
│   │   └── ...
│   ├── Group2/
│   │   └── ...
│   └── Group3/
│       └── ...
├── Models/
│   └── ExamResultStorage.cs
├── References/
│   └── JSON/
│       ├── MOS演習問題文一覧_Word.json
│       ├── MOS模擬試験①問題文一覧_Word.json
│       └── MOS模擬試験②問題文一覧_Word.json
├── Ui/
│   └── ViewModels/
│       └── MainViewModel.cs
└── Views/
    ├── ReviewPageWindow.xaml / ReviewPageWindow.xaml.cs
    ├── ResultWindow.xaml / ResultWindow.xaml.cs
    ├── ScoringResultDialog.xaml / ScoringResultDialog.xaml.cs
    └── ImageWindow.xaml / ImageWindow.xaml.cs
```

## 実装手順（別プロジェクトとして作成）

1. **新しいフォルダとプロジェクトの作成**
   - 新しいフォルダ（例: `mos_word_app`）を作成
   - Excel版のプロジェクトをコピー
   - プロジェクト名を `MOSWordMogiApp` に変更

2. **プロジェクトファイル（.csproj）の変更**
   - `RootNamespace` と `AssemblyName` を変更
   - ExcelのCOM参照を削除し、WordのCOM参照を追加
   - `PublishUrl` などのパスを変更

3. **名前空間の一括変更**
   - すべてのC#ファイルで `MOSExcelMogiApp` → `MOSWordMogiApp` に変更
   - Visual Studioの「検索と置換」機能を使用（Ctrl+Shift+H）
...
5. **config.jsonの変更**
   - `excelFile` → `wordFile` に変更
   - ファイルパスを `.xlsx` → `.docx` に変更
   - `library` 名を `ExcelChecker` → `WordChecker` に変更

6. **コード内の変更**
   - `LaunchAndPositionExcel` → `LaunchAndPositionWord`
   - `ExcelApp` → `WordApp`
   - `ExcelWorkbook` → `WordDocument`
   - プロセス名 `EXCEL` → `WINWORD`
   - ウィンドウクラス名 `XLMAIN` → `OpusApp`

7. **JSONファイルの準備**
   - Word用の問題文JSONファイルを作成
   - `References/JSON/` フォルダに配置

8. **Checkerクラスの準備**
   - `Libraries/Group1/WordChecker1_2.cs` などを作成（のちほど実装）
   - チェックメソッド名: `CheckTask_1_2_01()`, `CheckTask_1_2_02()` など（Excel版と同じ命名規則）

## 依存関係（NuGetパッケージ）

以下のNuGetパッケージが必要です：
- **Newtonsoft.Json** (Version 13.0.3 以上)
- **Microsoft.Office.Interop.Word** (COM参照として追加)

## Word特有の注意事項

- Wordのウィンドウクラス名は通常 `OpusApp` ですが、環境によって異なる可能性があります
- Wordのドキュメントは `Documents` コレクションで管理されます（Excelの `Workbooks` に相当）
- Wordのアクティブドキュメントは `ActiveDocument` プロパティで取得できます
- Wordファイルのパスは `Document.FullName` で取得できます
- WordのCOMオブジェクト解放時は、Excelと同様に `Marshal.ReleaseComObject()` を使用してください
...
**実装方法**:
- CSVファイルを`References/CSV/`フォルダに配置（例: `Word問題数一覧.csv`）
...
- **問題数の設定**: Word版の各プロジェクトの問題数は、Excel版と異なる可能性があります。`config.json`の`taskCount`や、アプリバーのタスク番号ボタンの数は、**Word版の実際の問題数に合わせて設定してください**。問題数はCSVファイルで管理されるため、CSVファイルから読み込んだ値を使用してください。
- WordCheckerはのちほど作成するため、Checkerクラスの実装は後回しでOK
- UIの配置やボタンの機能はExcel版と完全に同じにする
- タイマー機能、レビューページ機能、結果画面機能はすべて同じロジックを使用
- アプリバーのウィンドウ位置やサイズもExcel版と同じ（1920x258、Y=774）
- Wordのウィンドウクラス名は環境によって異なる可能性があるため、複数のパターンを確認する
