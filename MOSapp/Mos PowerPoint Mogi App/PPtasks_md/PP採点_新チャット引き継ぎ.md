...
**最終更新**: 2026-06-23（P7 完了・P8 Phase A 完了）
...
| **P7** | **完了・検証済** | P7-1〜5 連続採点○。P7-2 は免除なし（リンク設定のみ） |
| **P8** | **Phase A 完了** | Checker スタブ・Grader 01-05・config 付替・問題文 JSON |
| P9〜P11 | 未着手 | |
...
## 7. 完了: プロジェクト7（P7）最終仕様メモ

- P7-1〜P7-5 を一連で操作・採点 → **全タスク ○**
- **P7-2**: `TryHyperlinkOnRunsContainingText` で無限ループ回避。免除は **なし**（TextLength・ShapePosition 削除）
- **P7-4**: スライド **2・3・4 必須**、5以降は判定しない（`SlideHasP7_4Footer`）
...
| **Phase B** | タスク単位: Legacy 移植 → VSTO（要否）→ 検証 → 次タスク | P8-1 から順 |

**Phase B 推奨順**: P8-1 → P8-2 → P8-3 → P8-4 → P8-5

### 8.1 タスク一覧（新番号）

`projectId = 8`。`PowerPointChecker1_8.cs` を **P8 用に上書き**する（旧8-1〜5 動画/音声/読み取り専用は新P8に含まれない → P9 等へ）。

| 新タスク | 旧タスク | 移植元 Legacy | 概要 |
|----------|----------|---------------|------|
| P8-1 | 旧9-2 | `PowerPointChecker1_9.Legacy.cs` `CheckTask_1_9_02` | 表スタイル「中間スタイル4-アクセント4」・行交互なし |
| P8-2 | 旧6-2 | `PowerPointChecker1_6.Legacy.cs` `CheckTask_1_6_02` | スライド1背景「濃い緑、テキスト2、白+基本色80%」 |
| P8-3 | 旧11-1 | `PowerPointChecker1_11.Legacy.cs` `CheckTask_1_11_01` | スライドサイズ16:9 |
| P8-4 | 旧9-7 | `PowerPointChecker1_9.Legacy.cs` `CheckTask_1_9_07` | 高さ17.46cm・幅27.54cm・画面に合わせる |
| P8-5 | 旧10-4 | `PowerPointChecker1_10.Legacy.cs` `CheckTask_1_10_04` | 表示グレースケール + スライド2イラスト明るいグレースケール |

### 8.2 破壊的操作免除（Phase A 設定済み）

| タスク | 免除 | 備考 |
|--------|------|------|
| P8-1, P8-2, P8-5 | なし | |
| P8-3 | Shapes, Position | 旧11-1 から移植 |
| P8-4 | Position | 旧9-7 から移植 |

### 8.3 Phase B で注意する点

| タスク | メモ |
|--------|------|
| P8-5 | 旧10-4 は VSTO 証跡 `[Task10-4] Grayscale` + `BlackWhiteMode`。P8 用タグ拡張の可能性 |
| P8-4 | cm→pt 換算で `PageSetup` 比較（Legacy 9-7 参照） |
| P8-2 | P6-1 のコメント削除（旧10-1）とは別物。背景色判定は Legacy 6-2 |
