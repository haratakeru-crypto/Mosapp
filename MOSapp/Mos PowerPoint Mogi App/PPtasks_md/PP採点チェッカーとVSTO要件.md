...
- **背景**: 日本語版 PowerPoint で正しい操作（例: 画面切り替え「右から」、アニメーション「ワイプアウト（横）」）を行っても、環境やバージョンにより保存される COM 値に混乱が生じることがある。
...
- **採点側の対応**:
    ...
    - 以前は 3853 と 3854 の両方を許容していたが、デバッグ出力により環境ごとの値を特定した上で、厳密な判定へ移行推奨。
